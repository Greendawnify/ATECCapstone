...
using System.Collections;
    ...
    private void OnEnable()
    {
        if (!firstTime)
        {
            ...
            if (projectileParticle)
            {
                projectile = Instantiate(projectileParticle, transform.position, transform.rotation) as GameObject;
    ...
    void FixedUpdate()
        {	
            ...
            if (Physics.SphereCast(transform.position, radius, direction, out hit, detectionDistance, blockBullet)) // Checks if collision will happen
            {
            ...
            for (int i = 1; i < trails.Length; i++) // Loop to cycle through found particle systems
                {
                    ...
                    if (trail.gameObject.name.Contains("Trail"))
                    {
                        ...
                        //StartCoroutine(TurnOffTrail(trail.gameObject));
                    }
                }
                if (projectileParticle)
                {
                    Destroy(projectile, 3f); // Removes particle effect after delay
                    //Invoke("RechildProjectileFX", 3f);
                }
                Destroy(impactP, 3.5f); // Removes impact effect after delay
                //StartCoroutine(TurnOffFX(impactP, 3.5f));
        ...
        void RechildProjectileFX() {
            projectileParticle.SetActive(false);
            projectileParticle.transform.position = Vector3.zero;
        }

        IEnumerator TurnOffTrail(GameObject go) {
            yield return new WaitForSeconds(2f);
            go.transform.SetParent(transform);
            go.SetActive(false);
        }

        IEnumerator TurnOffFX(GameObject fx, float time) {
